...
| `10-clarification-reply-resume-marker.json` | reply coordinator loop | A coordinator re-entering as a clarification REPLY (`agent.run.entity_id` + `agent.loop.reply_to` both present) stamps `agent.run.clarification_resumed=$entity.instance` on the run entity. Resume marker, half 1 — single rule (no anchor split: semstreams#256 threads the run anchor onto the reply). **Active on beta.106.** (Phase 4b-2 PR-2.) |
| `11-resume-awaiting-to-executing.json` | run entity | `agent.run.clarification_resumed` + `phase==awaiting_approval` → `executing`, then `remove clarification_pending`, then `remove clarification_resumed` (in that order). Resume transition + marker-clear, half 2. **Active on beta.106.** (Phase 4b-2 PR-2.) |
...
`10` fires on that reply loop and stamps `agent.run.clarification_resumed` on the
...
`agent.run.entity_id`; the `agent.loop.reply_to` discriminator is what separates a
...
run anchor (`HTTPMessageRequest.run_id → TaskMessage.RunID → agent.run.entity_id`)
and the reply identity (`in_reply_to → TaskMessage.InReplyTo → agent.loop.reply_to`,
...
fire on loop creation. The maintainer chose the **typed** `agent.loop.reply_to`
...
- **A production human-facing reply affordance** — 4b-2 resume (rules `10`/`11`) is
  ...
  `coordinator.user_question` with a free-text answer box that POSTs `run_id` +
