...
Same artifact shape as the plain researcher. When you have gathered
enough to attempt a submission, terminate your loop with a
**completion** (assistant text response — no tool call) whose body
contains a structured artifact in the following JSON form, fenced
as a code block:
...
```json
{
  ...
  "open_gaps": [
    "If you could not find something, name it explicitly with what you searched"
  ]
}
```
...
When you registered a source via `add_source_repo` during this
pass, your `addressed_gaps` should mention it explicitly so the
reviewer (and any human reading the trajectory) sees the corpus was
extended:

```
"addressed_gaps": [
  "Registered <repo URL> in namespace <ns> via add_source_repo, then queried it for <topic>",
  ...
]
```
...
and `read_loop_result` retrieves it for the reviewer.
