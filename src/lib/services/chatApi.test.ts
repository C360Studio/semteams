...
/** Minimal ChatRequest fixture. */
function makeChatRequest(userMessage = "build me a flow") {
  return {
    messages: [{ role: "user" as const, content: userMessage }],
    currentFlow: { nodes: [] as FlowNode[], connections: [] as FlowConnection[] },
  };
}

/** Default no-op callbacks. */
function makeCallbacks(overrides: Partial<ChatStreamCallbacks> = {}): ChatStreamCallbacks {
  return {
    onText: vi.fn(),
    onDone: vi.fn(),
    onError: vi.fn(),
...
describe("streamChat — request", () => {
  it("POSTs to /api/ai/chat", async () => {
    const stream = makeStream([sseEvent("done", JSON.stringify({ flow: null }))]);
  ...
  it("sends the ChatRequest body as JSON", async () => {
    ...
    expect(body.currentFlow).toBeDefined();
  });
...
describe("streamChat — onDone", () => {
  it("calls onDone when done event received", async () => {
    ...
    const stream = makeStream([sseEvent("done", JSON.stringify(doneData))]);
    mockFetch.mockResolvedValueOnce({ ok: true, body: stream });
  ...
  it("onDone receives flow attached to done event data", async () => {
    ...
    const stream = makeStream([sseEvent("done", JSON.stringify({ flow }))]);
    mockFetch.mockResolvedValueOnce({ ok: true, body: stream });

    const onDone = vi.fn();
    await streamChat(makeChatRequest(), makeCallbacks({ onDone }));

    expect(onDone.mock.calls[0][0].flow).toEqual(flow);
  });

  it("onDone receives validationResult when present", async () => {
    const validationResult = { valid: false, errors: ["missing output"] };
    const stream = makeStream([sseEvent("done", JSON.stringify({ flow: null, validationResult }))]);
    mockFetch.mockResolvedValueOnce({ ok: true, body: stream });

    const onDone = vi.fn();
    await streamChat(makeChatRequest(), makeCallbacks({ onDone }));

    expect(onDone.mock.calls[0][0].validationResult).toEqual(validationResult);
  });
...
describe("streamChat — cancellation", () => {
  it("does not call onError when the request is aborted (AbortError is not an error)", async () => {
    ...
    const onError = vi.fn();
    const promise = streamChat(makeChatRequest(), makeCallbacks({ onError }), controller.signal);
    controller.abort();
...
describe("streamChat — network failure", () => {
  it("rejects on network failure", async () => {
    ...
    await expect(streamChat(makeChatRequest(), makeCallbacks())).rejects.toThrow("Failed to fetch");
  });
