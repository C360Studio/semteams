...
// harnesses. The fragment is upserted into the PERSONAS KV bucket
...
//     entries (name, image, smoke schema, what the harness verifies).
//   - Catalog is empty: render a short "no harnesses currently
//     registered" notice. The static instructions fragment tells
//     the LLM to flag `needs_harness:` in open_gaps in this case.
//
...
func RenderResearcherFragment(catalog []*Harness) string {
	...
	if len(catalog) == 0 {
		...
		b.WriteString("- `needs_harness: real Meshtastic radio over LoRa for protobuf POSITION_APP packets`\n")
		b.WriteString("- `needs_harness: a Kafka cluster with the upstream's exact topic schema`\n")
		b.WriteString("- `needs_harness: an OPC-UA server exposing the vendor's address space`\n\n")
		b.WriteString("If the work does NOT need external integration verification (pure unit-testable logic, in-process algorithms), omit the `harness` field and DO NOT add a `needs_harness:` gap — that path is reserved for genuine integration gaps.\n")
		return b.String()
	}
	...
	b.WriteString("If NONE of the registered harnesses fits the work this artifact describes, leave `harness` unset and add a `needs_harness: <description>` line to `open_gaps`. The reviewer (and downstream coordinator routing) treats absence-with-gap as honest scoping; absence-without-gap is a reviewer rejection.\n")
	return b.String()
}
