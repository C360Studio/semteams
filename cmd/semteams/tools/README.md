...
| `emit_research_artifact` | [`emitartifact/`](emitartifact/) | Writes marker triples + publishes typed `research.artifact.v1` payload on a stable subject. Researcher persona calls it before `submit_work`. | [ADR-031 §R3.2 + addendum 2026-04-30](../../../docs/adr/031-research-flow-and-semspec-handoff.md) | Re-evaluate when upstream ships the planned generic `write_artifact` tool ([ADR-028 §What's not built here](https://github.com/c360studio/semstreams/blob/main/docs/adr/028-orchestration-architecture.md)). Verified absent in semstreams beta.27. |
