...
| `emit_bootstrap_execute` | [`emitbootstrapexecute/`](emitbootstrapexecute/) | Forward-stamps `sandbox.tenant.*` triples from the execute persona's spawn prompt onto the execute loop entity. Bridges the rule 03 (execute → verify) substitution gap — execute receives plan triples via rule 02b's prompt substitution but its own loop entity carries no `sandbox.tenant.*`, so the next hop's `$entity.triple.sandbox.tenant.*` substitution would otherwise resolve to literal tokens. Mirrors `emit_bootstrap_verify`'s pattern (each role stamps on its own loop entity for the next rule's substitution). Pure plumbing — no canonicalization, no registry mutation. | [ADR-042 §addendum 2026-05-29](../../../docs/adr/042-coordinator-instantiated-flows-via-templates.md) + [smoke #9 finding 1](../../../../.claude/projects/-Users-coby-Code-c360-semteams/memory/project_smoke9_findings.md) | Retire when upstream lands a substitution form that traverses from `$entity` to related/lineage entities (e.g. `$lineage.<key>.triple.X` or `$related.<key>.triple.X`) — see [smoke #9 Option C](../../../../.claude/projects/-Users-coby-Code-c360-semteams/memory/project_smoke9_findings.md). At that point rule 03 can substitute directly from the run entity's `emit_bootstrap_plan` stamp; this tool becomes redundant. |
| `emit_bootstrap_plan` | [`emitbootstrapplan/`](emitbootstrapplan/) | Terminal tool of `provisioner-bootstrap-plan`. Accepts **structured intent** (source kind, dependency kinds, mount specs, smoke contract). The Go composer at `sandboxfleet.Compose` deterministically renders the actual shell strings (`git clone` with `--branch` before URL, `apt-get update && apt-get install -y --no-install-recommends ...`, `semteams-tenant-<prefix>-<suffix>:<path>` volume specs). LLM never authors CLI grammar — that's deterministic given intent and lives in code. Tool canonicalizes the signature, composes the recipe, computes `plan_hash` over composed strings, dual-target stamps `sandbox.tenant.*` triples on run + plan-loop entities, transitions registry to `provisioning`. | [ADR-042 §addendum PR 3.3](../../../docs/adr/042-coordinator-instantiated-flows-via-templates.md) + [smoke #10 findings](../../../../.claude/projects/-Users-coby-Code-c360-semteams/memory/project_smoke10_findings.md) | Stays product-local. The structured-intent composer is sandbox-bootstrap-pack policy; upstream has no equivalent. If a sibling product needs the same "structured intent → deterministic shell" pattern, graduate the composer to a shared package. |
