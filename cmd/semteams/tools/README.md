...
| `emit_curator_artifact` | [`emitcuratorartifact/`](emitcuratorartifact/) | source-curator role's terminal handoff. Publishes typed `curator.artifact.v1` payload on `curator.artifact.{loop_id}` and mints 5 marker triples on the curator's loop entity (`added_sources_count`, `verified_entity_ids_count`, `source_dirs_count`, `namespaces`, `generated_at`). No markdown render — the artifact is runtime data consumed by the next researcher via `read_loop_result`. The payload carries newly-indexed source repos, verified entity IDs, and optional shared source-dir mount paths (ADR-040 addendum 2026-05-10 §item 3). | [ADR-040](../../../docs/adr/040-source-curator-role.md) | Stays product-local. Curator concept is SemTeams-specific; semstreams has no upstream primitive. Per ADR-040 §"`emit_curator_artifact` typed payload" — explicit framework-alignment carve-out. |
