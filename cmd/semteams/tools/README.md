...
| `render_openspec` | [`renderopenspec/`](renderopenspec/) | The read/render half of the OQ1 OpenSpec tool pair (ADR-057 §D5 + addendum). Reads the `change.<slug>.*` triples a create_change arc stamped on the run entity (resolved from the call's run anchor), reconstructs the change via the pure `openspec.ChangeFromFacts`, and renders it to a single OpenSpec-markdown document (`openspec.RenderChangeFolder` — proposal + per-capability delta specs + tasks under `openspec/changes/<slug>/` file markers). Auto-discovers the single change on the entity when `slug` is omitted. The create-change wake-up coordinator (rule 03) calls it to deliver the actual reviewed spec to the user, not just a prose summary. Thin graph adapter; all format logic stays in the dep-free `openspec` package. Writer-only graph-state (`status`/`acceptance_command`/§D6 task fields) is not modeled by `ChangeFromFacts`, so it never leaks into the markdown. | [ADR-057](../../../docs/adr/057-openspec-graph-spec-model-and-create-change.md) | Same migration target as `emit_change` — the planned generic `read_artifact`/`write_artifact` suite ([ADR-028 §What's not built here](https://github.com/c360studio/semstreams/blob/main/docs/adr/028-orchestration-architecture.md)), verified absent in semstreams beta.114. The UC-1 extension (write the rendered folder into the sandbox workspace for a PR) is deferred — it needs a sandbox FS writer + a PR flow not present in P2. |
| `analyze_proof_readiness` | [`analyzeproof/`](analyzeproof/) | Deterministic spec-driven-development gate. Reads `proof.*` facts from the current run entity and atomically emits `formal_claims.*` findings on that same entity: no accepted claims, missing dependencies, stale/failed readiness, expired/revoked waivers, and ready evidence. It does not ask an LLM to judge infrastructure; it projects graph facts into routeable blocker/warning facts for rules, coordinator wake-up, and the UI's "is this thing working?" surface. | OpenSpec change `spec-driven-dev-readiness-hitl` §2.2 | Stays product-local until the pattern proves framework-generic. It composes existing upstream primitives (`read_entity`/graph read shape + `TriplePublisher.AddTriplesBatch` from `decide`/`emit_diagnosis`) rather than adding a new state store. If semstreams grows a generic graph-fact analyzer or artifact-readiness gate, migrate this analyzer to that primitive and keep the `formal_claims.*` contract as SemTeams policy. |
| `project_spec_tasks` | [`projectspecplan/`](projectspecplan/) | Deterministic dev-from-task adapter. Reads a reviewer-approved OpenSpec change from the current run entity (via run anchor or `related_loops["run-loop-entity-id"]`), validates that every task has the execution-rich §D6 fields, and stamps the exact `plan.*` / `plan.task.*` contract consumed by the existing dev-via-test coordinator walk, Ralph loop, and CBG gate. The chain-level `change.<slug>.acceptance_command` becomes `plan.integration_test_command`; task IDs are deterministic (`task-0`, `task-1`, ...); `plan.done_authority.*` records that the approved OpenSpec change owns done and CBG is the final gate. It is intentionally not a planner and does not alter Lisa/dev-via-test; the dev-from-task rule pack creates the chain-start git tag before dispatching Ralph. | OpenSpec change `spec-driven-dev-readiness-hitl` §3.1 + [ADR-057 §D6](../../../docs/adr/057-openspec-graph-spec-model-and-create-change.md#d6-task--execution-superset) | Same migration target as the OpenSpec tool pair: upstream's future artifact/read-projection primitives. Until then it composes the existing graph read + batch triple-publish surface and keeps the projection policy product-local. |
