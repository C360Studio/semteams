...
func New(deps ComponentDeps) *Component {
	...
	return &Component{
		...
		metrics:    metrics,
		startTime:  time.Now(),
		batch:      make([]trustgraph.TGTriple, 0, deps.Config.BatchSize),
	}
}
...
func (c *Component) translateTriples(triples []message.Triple) []trustgraph.TGTriple {
	vocabTriples := c.translator.TriplesToRDF(triples)

	// Convert vocab.TGTriple to trustgraph.TGTriple
	result := make([]trustgraph.TGTriple, len(vocabTriples))
	for i, vt := range vocabTriples {
		result[i] = trustgraph.TGTriple{
			S: trustgraph.TGValue{V: vt.S.V, E: vt.S.E},
			P: trustgraph.TGValue{V: vt.P.V, E: vt.P.E},
			O: trustgraph.TGValue{V: vt.O.V, E: vt.O.E},
		}
	}
	return result
}
...
func (c *Component) flush(ctx context.Context) {
	...
	// Take the batch
	batch := c.batch
	c.batch = make([]trustgraph.TGTriple, 0, c.config.BatchSize)
