...
Detailed specifications for the 9 graph processing components in SemStreams.

## Overview

The SemStreams graph subsystem decomposes into 9 specialized components with clear ownership boundaries:
...
│                    ┌──────────────────┼──────────────────┐          │
│                    ▼                  ▼                  ▼          │
│              graph-index      graph-anomalies    graph-clustering   │
│                    │          graph-embedding    graph-index-*      │
│                    ▼                                                 │
│            Index Buckets ◄─────────────────────────────────────┐    │
│                    │                                            │    │
│                    └──────────────► graph-query ◄───────────────┘    │
...
### 3. graph-anomalies - Structural Analysis

**Purpose**: Performs graph-theoretic analysis (k-core decomposition, pivot-based distances) to identify structural
anomalies and central entities.

**Interfaces**: `Discoverable`, `LifecycleComponent`, `QueryCapabilityProvider`

**Input Ports**:

- `outgoing_watch` (kv-watch): Watches `OUTGOING_INDEX`
- `incoming_watch` (kv-watch): Watches `INCOMING_INDEX`
...
```json
{
  ...
  "min_entities_for_analysis": 100,
  "ports": {
    "inputs": [
      {"name": "outgoing_watch", "type": "kv-watch", "bucket": "OUTGOING_INDEX"},
      {"name": "incoming_watch", "type": "kv-watch", "bucket": "INCOMING_INDEX"}
    ],
    "outputs": [
...
| Subject | Operation | Description |
|---------|-----------|-------------|
| `graph.anomalies.query.kcore` | getKCore | Get k-core number for entity or entities by min core |
| `graph.anomalies.query.pivot` | getPivotDistances | Get pivot distance vector for entity |
| `graph.anomalies.query.outliers` | findOutliers | Find structural outliers (isolation, demotion) |
| `graph.anomalies.capabilities` | capabilities | Discover query capabilities |
...
**Execution**: Batch processing on configurable interval (default: 1 hour)

---

### 4. graph-clustering - Community Detection

**Purpose**: Groups entities into communities using Label Propagation Algorithm (LPA), enabling GraphRAG-style
queries at different granularity levels.
...
**Algorithm**: Label Propagation Algorithm (LPA) with convergence detection

**Execution**: Triggered by threshold of entity changes or time interval

---

### 5. graph-embedding - Vector Embeddings
...
### 6. graph-index-spatial - Geospatial Indexing
...
### 7. graph-index-temporal - Time-Based Indexing
...
- `STRUCTURAL_INDEX`, `ANOMALY_INDEX` (via graph-anomalies queries)
- `COMMUNITY_INDEX` (via graph-clustering queries)
...
### 9. graph-query - Query Coordinator
...
| `OUTGOING_INDEX` | graph-index | graph-anomalies, graph-gateway | Entity → referenced entities |
| `INCOMING_INDEX` | graph-index | graph-anomalies, graph-gateway | Entity → referencing entities |
...
| `STRUCTURAL_INDEX` | graph-anomalies | graph-gateway | K-core levels and pivot distances |
| `ANOMALY_INDEX` | graph-anomalies | graph-query, graph-gateway | Anomaly detection results |
| `COMMUNITY_INDEX` | graph-clustering | graph-gateway | Community records with members |
| `EMBEDDING_INDEX` | graph-embedding | graph-gateway | Entity ID → embedding vector |
...
**Adds**: K-core analysis, structural anomaly detection, coordinated queries
...
**Adds**: Community detection, BM25 semantic similarity
...
**Adds**: Neural embeddings, LLM-powered summarization
...
| graph-anomalies | Batch | Configurable (default: 1h) | Periodic batch |
...
- **graph-anomalies**: Scale for batch processing workload
- **graph-clustering**: Scale for community detection workload
...
- graph-anomalies (k-core decomposition)
- graph-clustering (LPA iterations)
- graph-embedding (BM25 computation)

**Memory-intensive components**:

- graph-anomalies (in-memory graph for analysis)
- graph-clustering (community membership tracking)
...
- `analyses_completed_total` (graph-anomalies)
...
| graph-anomalies | Structural data stale | Next batch analysis updates |
| graph-clustering | Communities stale | Next detection cycle updates |
