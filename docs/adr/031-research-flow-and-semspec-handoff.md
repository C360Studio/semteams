...
## Subsequent decisions moved to ADR-035 + ADR-036

Addenda 2026-04-30 (R3.3 dev-via-spec port pattern-source survey),
2026-05-02 (R3.4a smoke #4 + R3.5 design target), and 2026-05-02
(R3.4b smoke #5 chain-converged) were originally appended here.
They covered the post-research dev-via-spec arc — chain shape,
persona contracts, substance-over-format pivot. Those decisions
have been consolidated into [ADR-035](035-dev-via-spec-arc.md) so
this ADR can stay scoped to research flow ownership and the
SemSpec handoff decision it was originally written for.

[ADR-036](036-test-harness-lifecycle.md) owns the verification
machinery (test-harness lifecycle) that the post-builder slice
needs and that smoke #7 surfaced as missing.

The strategic pivot driving the split is recorded in memory at
`project_strategic_pivot_2026_05_06.md`.
