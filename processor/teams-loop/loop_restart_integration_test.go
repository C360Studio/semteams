...
func newLoopTestConfig(suffix string) teamsloop.Config {
...
func startLoopComponent(t *testing.T, ctx context.Context, config teamsloop.Config) component.LifecycleComponent {
	t.Helper()
	...
	comp, err := teamsloop.NewComponent(rawConfig, component.Dependencies{NATSClient: natsClient})
	require.NoError(t, err)
