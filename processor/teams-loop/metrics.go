// Package teamsloop provides Prometheus metrics for agentic-loop component.
package teamsloop
...
func getMetrics(registry *metric.MetricsRegistry) *loopMetrics {
	metricsOnce.Do(func() {
		...
		if registry != nil {
			_ = registry.RegisterCounter("teams-loop", "loops_created_total", metrics.loopsCreated)
			_ = registry.RegisterCounter("teams-loop", "loops_completed_total", metrics.loopsCompleted)
			_ = registry.RegisterCounterVec("teams-loop", "loops_failed_total", metrics.loopsFailed)
			_ = registry.RegisterCounter("teams-loop", "loops_timeout_total", metrics.loopsTimeout)
			_ = registry.RegisterGauge("teams-loop", "active_loops", metrics.activeLoops)
			_ = registry.RegisterCounter("teams-loop", "iterations_total", metrics.iterationsTotal)
			_ = registry.RegisterHistogram("teams-loop", "iterations_per_loop", metrics.iterationsPerLoop)
			_ = registry.RegisterHistogramVec("teams-loop", "duration_seconds", metrics.loopDuration)
			_ = registry.RegisterCounterVec("teams-loop", "trajectory_steps_total", metrics.trajectorySteps)
			_ = registry.RegisterCounterVec("teams-loop", "tool_calls_dispatched_total", metrics.toolCallsDispatched)
			_ = registry.RegisterCounterVec("teams-loop", "tool_results_received_total", metrics.toolResultsReceived)
			_ = registry.RegisterCounterVec("teams-loop", "boid_signals_received_total", metrics.boidSignalsReceived)
			_ = registry.RegisterCounter("teams-loop", "boid_position_updates_total", metrics.boidPositionUpdates)
			_ = registry.RegisterCounter("teams-loop", "boid_entities_filtered_total", metrics.boidEntitiesFiltered)
			_ = registry.RegisterGauge("teams-loop", "context_utilization", metrics.contextUtilization)
			_ = registry.RegisterCounter("teams-loop", "context_compactions_total", metrics.contextCompactionsTotal)
			_ = registry.RegisterHistogram("teams-loop", "context_compaction_tokens_saved", metrics.contextCompactionTokensSaved)
			_ = registry.RegisterGauge("teams-loop", "context_compacted_region_tokens", metrics.contextCompactedRegionTokens)
		} else {
