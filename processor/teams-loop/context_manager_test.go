...
func TestContextManager_Utilization(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			addMessages: map[teamsloop.RegionType][]teams.ChatMessage{
				...
				teamsloop.RegionRecentHistory: {
					{Role: "user", Content: "Hello"},
					{Role: "assistant", Content: "Hi there!"},
				},
				teamsloop.RegionToolResults: {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cm := teamsloop.NewContextManager("loop-1", tt.model, config)
...
func TestContextManager_ShouldCompact(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			targetTokens := int(float64(teamsloop.DefaultContextLimit) * tt.fillToUtilPct)
...
func TestContextManager_GetRegionTokens(t *testing.T) {
	...
	systemTokens := cm.GetRegionTokens(teamsloop.RegionSystemPrompt)
	recentTokens := cm.GetRegionTokens(teamsloop.RegionRecentHistory)
	toolTokens := cm.GetRegionTokens(teamsloop.RegionToolResults)
...
func TestContextManager_ModelLimits(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantLimitMin: teamsloop.DefaultContextLimit,
			wantLimitMax: teamsloop.DefaultContextLimit,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var opts []teamsloop.ContextManagerOption
			if tt.registry != nil {
				opts = append(opts, teamsloop.WithModelRegistry(tt.registry))
			}
			cm := teamsloop.NewContextManager("loop-1", tt.model, config, opts...)
...
func TestContextManager_MultipleMessagesPerRegion(t *testing.T) {
	...
	for _, msg := range messages {
		if err := cm.AddMessage(teamsloop.RegionRecentHistory, msg); err != nil {
			t.Fatalf("AddMessage() error = %v", err)
		}
	}
...
func TestContextManager_HeadroomReducesEffectiveCapacity(t *testing.T) {
	...
	cmWithHeadroom := teamsloop.NewContextManager("loop-h", "gpt-4o", config)
	cmNoHeadroom := teamsloop.NewContextManager("loop-nh", "gpt-4o", configNoHeadroom)

	// Fill both to the same number of tokens
	targetTokens := int(float64(teamsloop.DefaultContextLimit) * 0.55)
...
func TestContextManager_HeadroomTriggersCompactionEarlier(t *testing.T) {
	...
	fillPct := 0.58
	targetTokens := int(float64(teamsloop.DefaultContextLimit) * fillPct)
...
func TestContextManager_HeadroomExceedsModelLimit(t *testing.T) {
	...
	config.HeadroomTokens = teamsloop.DefaultContextLimit + 1000 // floor > model limit

	cm := teamsloop.NewContextManager("loop-edge", "gpt-4o", config)
...
func TestContextManager_ModelLimit(t *testing.T) {
	...
	if cm.ModelLimit() != teamsloop.DefaultContextLimit {
		t.Errorf("ModelLimit() = %d, want %d", cm.ModelLimit(), teamsloop.DefaultContextLimit)
	}
}

// Helper function to fill context to approximately target token count
func fillContextToTokens(t *testing.T, cm *teamsloop.ContextManager, targetTokens int) {
	t.Helper()
